...
$timestamp = strip_tags(trim(xss_clean(mysqli_real_escape_string($con, $_POST['timestamp']))));
...
$incorrectdetails = Encrypt(json_encode(array("status" => "failed", "info" => "Incorrect username or password.")));
$userbanned = Encrypt(json_encode(array("status" => "failed", "info" => "Your account has been banned!")));
$incorrecthwid = Encrypt(json_encode(array("status" => "failed", "info" => "Incorrect machine ID.")));
$timeexpired = Encrypt(json_encode(array("status" => "failed", "info" => "Your time has expired!")));
$nullentry = Encrypt(json_encode(array("status" => "failed", "info" => "Please fill in all fields before attempting to login!")));
$resethwid = Encrypt(json_encode(array("status" => "failed", "info" => "Your HWID has been reset, please login again.")));
...
if(strtolower($username) == strtolower($user) && (password_verify($password, $pass))){ //check username and pass after all checks are done..
      ...
      die($userbanned);
        ...
        if ($hwid == $hwidd || $hwid === $hwidd){ //hwid matches, carry on...
         $success = Encrypt(json_encode(array("status" => "success", "info" => "Successfully logged in!", "username" => $user, "email" => $email, "level" => $level, "expires" => $expires, "hwid" => $hwidd, "ip" => $ip, "timestamp" => $timestamp)));
         die($success);
        }
